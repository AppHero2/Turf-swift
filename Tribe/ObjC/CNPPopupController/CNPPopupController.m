...
#define COLOR_OVERLAY [UIColor colorWithRed:211.0/255.0 green:226.0/255.0 blue:239.0/255.0 alpha:0.5]
...
- (instancetype)initWithContents:(NSArray <UIView *> *)contents {
    ...
    if (self) {
        ...
        self.popupView.layer.shadowOffset = CGSizeMake(0, 0);
        self.popupView.layer.shadowOpacity = 0.12f;
        self.popupView.layer.shadowRadius = 5.0f;
        
        self.maskView = [[UIView alloc] initWithFrame:self.applicationWindow.bounds];
        self.maskView.backgroundColor = COLOR_OVERLAY;
...
- (void)applyTheme {
    ...
    if (self.theme.popupStyle == CNPPopupStyleFullscreen) {
        ...
        maskBackgroundColor = self.theme.maskType == CNPPopupMaskTypeClear?[UIColor clearColor] : COLOR_OVERLAY;
    }
...
//            self.popupView.center = CGPointMake(self.popupView.center.x, (CGRectGetHeight(self.maskView.frame) - maskViewIntersection.size.height) / 2);
            self.popupView.center = CGPointMake(self.popupView.center.x,
                                                (self.maskView.frame.size.height - keyboardFrame.size.height+keyboardFrame.origin.y)/2);
        } completion:nil];
    }
}
